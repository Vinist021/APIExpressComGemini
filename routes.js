...
router.post("/chat", async (req, resp) => {
    ...
    const response = await model.invoke("conte uma piada");
...
router.post("/summarize", upload.single("file"), async (req, resp) => {
    ...
    const prompt = ChatPromptTemplate.fromMessages([
        {role: "system",
             content: "você é um assistente que extrai dados estruturados de especificações de construção, retorne sempre um JSON válido."},
        {
